...
<!-- This file was auto-generated by get_docs.py and pandoc from er_settings.py -->
...
"major", "minor", "diminished", and so on. A generic interval together
with a quality, like "minor third", is somewhat like a specific
...
-   [**`num_voices`**]{#num_voices}: int. The number of "voices" (or
    "parts") to be created. If
...
-   [**`num_reps_super_pattern`**]{#num_reps_super_pattern}: int. Number
    of times to repeat the complete "super pattern".

    *Default*: `2`

-   [**`pattern_len`**]{#pattern_len}: a number, or a [per-voice
...
-   [**`truncate_patterns`**]{#truncate_patterns}: bool. If True, then
    repetitions of any values in
...
-   [**`rhythm_len`**]{#rhythm_len}: a number, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`harmony_len`**]{#harmony_len}: a number, or a looping sequence
    ...
    cycle through the sequence until
...
-   [**`scales`**]{#scales}: a sequence of sequences of numbers. Each
...
-   [**`chords`**]{#chords}: a sequence of sequences of numbers. Each
...
-   [**`voices_separate_tracks`**]{#voices_separate_tracks}: bool. If
    True, different "voices" in the score are written to separate tracks
    in the output midi file.

    *Default*: `True`

-   [**`choirs_separate_tracks`**]{#choirs_separate_tracks}: bool. If
    True, different "choirs" in the score are written to separate tracks
    in the output midi file.

    *Default*: `True`

-   [**`choirs_separate_channels`**]{#choirs_separate_channels}: bool.
    If True, different "choirs" in the score are assigned to separate
    channels in the output midi file (up to the maximum of 16 channels).

    *Default*: `True`

-   [**`write_program_changes`**]{#write_program_changes}: bool. If
    True, General Midi program changes are written to the output midi
    file. Depending on the intended use, this may or may not be desired.

    *Default*: `True`

-   [**`humanize`**]{#humanize}: bool. Whether to apply "humanization"
    to various parameters according to the various "humanize" settings
    below.

    *Default*: `True`

-   [**`humanize_attack`**]{#humanize_attack}: float. Randomly varies
    attack times within +- this amount, in quarter notes.

    *Default*: `0.0`

-   [**`humanize_dur`**]{#humanize_dur}: float. Randomly varies duration
    times within +- this amount, in quarter notes.

    *Default*: `0.0`

-   [**`humanize_velocity`**]{#humanize_velocity}: float between 0
    and 1. Scales velocities by 1 +- a random amount between 0 and this
    amount.

    *Default*: `0.1`

-   [**`humanize_tuning`**]{#humanize_tuning}: float. Only implemented
    when `tet != 12`. Randomly varies tuning within +- this amount.

    *Default*: `0.0`

### Tuning settings

-   [**`integers_in_12_tet`**]{#integers_in_12_tet}: bool. If `True`,
    then any pitch materials (e.g., in
...
-   [**`logic_type_pitch_bend`**]{#logic_type_pitch_bend}: bool. If
    True, turns on a midi-writing scheme that I devised in order to make
    non-12-tet files work in Logic Pro. That is, in order to avoide
    pitch-bend latency problems, as well as to avoid audible bends
    during the release tails of notes, the notes (and their associated
    pitch-bend events) of each voice loop through a number of different
    channels (set by
...
-   [**`num_channels_pitch_bend_loop`**]{#num_channels_pitch_bend_loop}:
...
-   [**`pitch_bend_time_prop`**]{#pitch_bend_time_prop}: number between
    0 and 1. If
    <a href="#logic_type_pitch_bend">`logic_type_pitch_bend`</a> is
...
-   [**`parallel_voice_leading`**]{#parallel_voice_leading}: bool. If
    `True`, then the voice-leading between harmonies is conducted in
    pure (generic) parallel motion.

    *Default*: `False`

-   [**`parallel_direction`**]{#parallel_direction}: int. Only has an
    effect if
...
-   [**`voice_lead_chord_tones`**]{#voice_lead_chord_tones}: bool. If
    True, then chord-tones on each harmony are voice-led to chord-tones
    on the next harmony, and non-chord-tones to non-chord-tones. So, if
    moving from a C major chord to an F major chord, the pitch-class C
    will be mapped to one of the pitch-classes (F, A, C), the
    pitch-class E will be mapped to one of the two remaining
    pitch-classes of the F major chord, and the pitch-class G will be
    mapped to the remaining pitch-class of the F major chord. A similar
    mapping will be made among the non-chord tones of the scale.
...
-   [**`constrain_voice_leading_to_ranges`**]{#constrain_voice_leading_to_ranges}:
...
-   [**`allow_flexible_voice_leading`**]{#allow_flexible_voice_leading}:
...
-   [**`vl_maintain_consonance`**]{#vl_maintain_consonance}: bool. If
    False, then after the initial pattern is complete, voice-leadings
    will not be checked for consonance. (See the settings under
    "<a href="#consonance-and-dissonance-settings">Consonance and
    dissonance settings</a>" below.)

    *Default*: `True`

-   [**`vl_maintain_limit_intervals`**]{#vl_maintain_limit_intervals}:
...
-   [**`vl_maintain_forbidden_intervals`**]{#vl_maintain_forbidden_intervals}:
    bool. If False, then after the initial pattern is complete,
...
-   [**`chord_tone_selection`**]{#chord_tone_selection}: boolean. If
    True, then the script will select whether each note should be
    assigned a chord-tone according to a probabilistic function (some of
    whose parameters can be set below). Note that even if this setting
    is True, however, and a particular note is assigned to be a
    chord-tone, if all chord-tones fail to satisfy the various
...
-   [**`chord_tone_prob_func`**]{#chord_tone_prob_func}: string. If
    <a href="#chord_tone_selection">`chord_tone_selection`</a> is True,
...
-   [**`max_n_between_chord_tones`**]{#max_n_between_chord_tones}: int.
    Controls after how many non-chord tones the probability of the
...
-   [**`min_prob_chord_tone`**]{#min_prob_chord_tone}: float. Sets the
    probability of a chord tone immediately following another chord
    tone. See
    <a href="#max_n_between_chord_tones">`max_n_between_chord_tones`</a>
    for an example.
...
-   [**`try_to_force_non_chord_tones`**]{#try_to_force_non_chord_tones}:
...
-   [**`len_to_force_chord_tone`**]{#len_to_force_chord_tone}: int. If
    <a href="#chord_tone_selection">`chord_tone_selection`</a> is True,
    then notes of this value or longer will be forced to be chord tones.
    To disable, set to 0.

    *Default*: `1`

-   [**`scale_chord_tone_prob_by_dur`**]{#scale_chord_tone_prob_by_dur}:
    bool. If True, and
    <a href="#chord_tone_selection">`chord_tone_selection`</a> is True,
...
-   [**`scale_chord_tone_neutral_dur`**]{#scale_chord_tone_neutral_dur}:
    number. If
...
-   [**`scale_short_chord_tones_down`**]{#scale_short_chord_tones_down}:
    bool. See
    <a href="#scale_chord_tone_prob_by_dur">`scale_chord_tone_prob_by_dur`</a>
    and
    <a href="#scale_chord_tone_neutral_dur">`scale_chord_tone_neutral_dur`</a>.
...
-   [**`chord_tone_before_rests`**]{#chord_tone_before_rests}: a number,
    or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`chord_tones_no_diss_treatment`**]{#chord_tones_no_diss_treatment}:
...
-   [**`force_chord_tone`**]{#force_chord_tone}: string. Possible
    values:
...
-   [**`chord_tones_sync_attack_in_all_voices`**]{#chord_tones_sync_attack_in_all_voices}:
...
-   [**`prefer_small_melodic_intervals`**]{#prefer_small_melodic_intervals}:
...
-   [**`prefer_small_melodic_intervals_coefficient`**]{#prefer_small_melodic_intervals_coefficient}:
    number. If
...
-   [**`unison_weighted_as`**]{#unison_weighted_as}: int. If
    prefer\_small\_melodic\_intervals is true, then we have to tell the
    algorithm how to weight melodic unisons, because we usually don't
    want them to be the most common melodic interval. Unisons will be
    weighted the same as whichever generic interval this variable is
    assigned to.
...
-   [**`max_interval_for_non_chord_tones`**]{#max_interval_for_non_chord_tones}:
...
-   [**`min_interval`**]{#min_interval}: number, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`force_repeated_notes`**]{#force_repeated_notes}: bool. If True,
    then within each harmony, each note is forced to repeat the pitch of
    the previous note.

    *Default*: `False`

-   [**`max_repeated_notes`**]{#max_repeated_notes}: integer. Sets the
...
-   [**`max_alternations`**]{#max_alternations}: integer, or a
    ...
    allowed, just not "A, B, A, B, A, B" (or longer). To disable, set
    to 0.

    *Default*: `2`

-   [**`pitch_loop`**]{#pitch_loop}: int, or a sequence of ints. If
    passed, in each voice, pitches will be repeated in a loop of the
    specified length. (However, at each harmony change, the loop will be
    adjusted to fit the new harmony.)

-   [**`hard_pitch_loop`**]{#hard_pitch_loop}: boolean. If True, then
    after the initial loop of each voice is constructed, pitch
    constraint parameters such as
...
-   [**`prohibit_parallels`**]{#prohibit_parallels}: sequence of
    numbers. The numbers will be treated as octave-equivalent intervals
    (so, e.g., an octave equals a unison). Parallel motion by these
    intervals will be forbidden. The obvious use is to prohibit parallel
    octaves, but can be used however one wishes. ([See the note above on
    specifying pitches and
    intervals](#note-on-specifying-pitches-and-intervals).)

    *Default*: `(OCTAVE,)`

-   [**`antiparallels`**]{#antiparallels}: bool. If True, then
    "antiparallel" versions of the intervals in
...
-   [**`force_parallel_motion`**]{#force_parallel_motion}: bool, or a
    dictionary where keys are tuples of ints (indicating voice indices)
    and values are bools. If True, then voices will be constrained to
    move in (generic) parallel motion. The parallel motion is only
    enforced within harmonies; across the boundaries between harmonies,
    voices may move freely. When this parameter is True between voices
    that do not have the same attacks, it works as follows: it takes the
    *last* melodic interval in the leader voice, and adds the same
    melodic interval in the follower voice.
...
-   [**`consonance_treatment`**]{#consonance_treatment}: string.
    Controls among which notes consonance is evaluated. Possible values:
...
-   [**`consonance_type`**]{#consonance_type}: string. Controls how
    notes are evaluated for consonance/dissonance. Possible values:
...
-   [**`consonance_modulo`**]{#consonance_modulo}: number, or a sequence
    of numbers, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
    ...
    consonance settings applied, but pitches *between* these beats will
    not.
...
-   [**`min_dur_for_cons_treatment`**]{#min_dur_for_cons_treatment}:
    number. Notes with durations shorter than this value will not be
    evaluated for consonance.

    *Default*: `0`

-   [**`forbidden_interval_classes`**]{#forbidden_interval_classes}: a
    sequence of numbers. The intervals in this sequence (interpreted as
    harmonic intervals) will be entirely avoided, regardless of
...
-   [**`forbidden_interval_modulo`**]{#forbidden_interval_modulo}:
...
-   [**`exclude_augmented_triad`**]{#exclude_augmented_triad}: boolean.
    Because all the pairwise intervals of the 12-tet augmented triad are
    consonant, if we want to avoid it, we need to explicitly exclude it.
    Only has any effect if consonance\_type is "pairwise".

    *Default*: `True`

-   [**`consonances`**]{#consonances}: a sequence of numbers. The
    sequence items specify the intervals that will be treated as
    consonances if <a href="#consonance_type">`consonance_type`</a> is
    `"pairwise"`. (But see
    <a href="#invert_consonances">`invert_consonances`</a> below.) Since
    it's just a sequence of numbers, you can specify any intervals you
...
-   [**`invert_consonances`**]{#invert_consonances}: bool. If True, then
    the contents of <a href="#consonances">`consonances`</a> are
    replaced by their setwise complement. (E.g., if
    <a href="#tet">`tet`</a> is 12, then `[0, 3, 4, 5, 7, 8, 9]` will be
    replaced by `[1, 2, 6, 10,           11]`.) This permits an easy way
    of specifying so-called "dissonant counterpoint". Has no effect if
    <a href="#consonance_type">`consonance_type`</a> is `"chordwise"`.

    *Default*: `False`

-   [**`consonant_chords`**]{#consonant_chords}: a sequence of sequences
    of numbers. Each sub-sequence specifies a chord to be understood as
    consonant if <a href="#consonance_type">`consonance_type`</a> is
    `"chordwise"`. ([See the note above on specifying pitches and
    intervals](#note-on-specifying-pitches-and-intervals).)

    *Default*: `(er_constants.MAJOR_TRIAD, er_constants.MINOR_TRIAD)`

-   [**`chord_octave_equi_type`**]{#chord_octave_equi_type}: string. If
    <a href="#consonance_type">`consonance_type`</a> is `"chordwise"`,
    ...
    pitch than E3, and not (E3, C3), because voice 0 is lower than
    voice 1.

    *Default*: `"all"`

-   [**`chord_permit_doublings`**]{#chord_permit_doublings}: string. If
    <a href="#consonance_type">`consonance_type`</a> is `"chordwise"`,
...
-   [**`rhythms_specified_in_midi`**]{#rhythms_specified_in_midi}:
...
-   [**`rhythms_in_midi_reverse_voices`**]{#rhythms_in_midi_reverse_voices}:
...
-   [**`attack_density`**]{#attack_density}: a float from 0.0 to 1.0, or
    an int, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`dur_density`**]{#dur_density}: a float from 0.0 to 1.0, or a
    [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`attack_subdivision`**]{#attack_subdivision}: a number, or a
    [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`sub_subdivisions`**]{#sub_subdivisions}: a sequence of ints, or
    a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`dur_subdivision`**]{#dur_subdivision}: a number, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`min_dur`**]{#min_dur}: a number, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`obligatory_attacks`**]{#obligatory_attacks}: a sequence of
    numbers, or a [per-voice
    sequence](#note-on-per-voice-sequences-and-other-looping-sequences)
...
-   [**`obligatory_attacks_modulo`**]{#obligatory_attacks_modulo}: a
    number, or a sequence of numbers. Specifies which times (if any)
...
-   [**`comma_position`**]{#comma_position}: string, int, or sequence of
    strings and/or ints. If the <a href="#rhythm_len">`rhythm_len`</a>
    is not divisible by
    ...
    "beginning": comma is placed at the beginning of the rhythm, before
    any notes. - "middle": comma is placed randomly in the middle of the
    rhythm. - int: specify the index at which to place the comma.

    *Default*: `"end"`

-   [**`overlap`**]{#overlap}: bool. If `True`, then the final durations
    of each pattern repetition can overlap with the first durations of
    the next. If `False`, pattern overlaps are avoided.

    *Default*: `True`

-   [**`rhythmic_unison`**]{#rhythmic_unison}: bool, or a sequence of
    tuples of ints. Controls whether to apply "rhythmic unison" (i.e.,
    simultaneous attacks and releases) to some or all voices.
...
-   [**`rhythmic_quasi_unison`**]{#rhythmic_quasi_unison}: bool, or a
    sequence of tuples of ints. This parameter is specified in the same
    way as <a href="#rhythmic_unison">`rhythmic_unison`</a> above.
    However, whereas <a href="#rhythmic_unison">`rhythmic_unison`</a>
    causes voices to have precisely the same rhythms, this parameter
    works differently. It does not override the rhythmic settings (such
    as <a href="#attack_density">`attack_density`</a>) that apply to
    each voice, but it constrains the attacks of "follower" voices to
    occur at the same time as the attacks of "leader" voices, as far as
    is possible. If the follower has a greater
...
-   [**`rhythmic_quasi_unison_constrain`**]{#rhythmic_quasi_unison_constrain}:
    boolean. If
...
-   [**`hocketing`**]{#hocketing}: bool, or a sequence of tuples of
    ints. This parameter is specified in the same way as
    <a href="#rhythmic_unison">`rhythmic_unison`</a> above. Its effect
    is to lead combinations of voices to be "hocketed" ---i.e., for,
    when possible, the attacks of one voice to be placed during the
    pauses of another, and vice versa.
...
-   [**`cont_rhythms`**]{#cont_rhythms}: string. Specifies whether and
    how to use "continuous" rhythms. Usually, rhythms in music are
    understood in a discrete manner, as rational subdivisions or
    combinations of unit values like quarter-notes or
...
-   [**`num_cont_rhythm_vars`**]{#num_cont_rhythm_vars}: int, or
    sequence of ints. If `cont_rhythms != "none"`, then, optionally, the
    rhythms can be varied when they recur by perturbing them randomly.
    This parameter determines the number of such variations. Setting it
    to `1` has the effect of disabling it. Setting it to a negative
    value will cause the rhythmic variations to continue throughout the
    super pattern, or throughout the entire output, depending on the
    value of
...
-   [**`vary_rhythm_consistently`**]{#vary_rhythm_consistently}: bool.
    If True, and `num_cont_rhythm_vars !=           1`, then the
...
-   [**`cont_var_increment`**]{#cont_var_increment}: number. If
    `num_cont_rhythm_vars != 1`, determines how much rhythmic
    perturbation is applied to each variation. Larger values lead to
    larger perturbations.

    *Default*: `0.1`

-   [**`super_pattern_reps_cont_var`**]{#super_pattern_reps_cont_var}:
...
-   [**`choirs`**]{#choirs}: a sequence of ints or tuples.
...
-   [**`choir_assignments`**]{#choir_assignments}: sequence of ints.
    Assigns voices to the given index in <a href="#choirs">`choirs`</a>.
    Will be looped through if necessary. For example, if
...
-   [**`randomly_distribute_between_choirs`**]{#randomly_distribute_between_choirs}:
...
-   [**`length_choir_segments`**]{#length_choir_segments}: number. Only
    has an effect if
    <a href="#randomly_distribute_between_choirs">`randomly_distribute_between_choirs`</a>
...
-   [**`length_choir_loop`**]{#length_choir_loop}: number. Only has an
    effect if
    <a href="#randomly_distribute_between_choirs">`randomly_distribute_between_choirs`</a>
...
-   [**`choir_segments_dovetail`**]{#choir_segments_dovetail}: bool.
...
-   [**`max_consec_seg_from_same_choir`**]{#max_consec_seg_from_same_choir}:
...
-   [**`all_voices_from_different_choirs`**]{#all_voices_from_different_choirs}:
...
-   [**`each_choir_combination_only_once`**]{#each_choir_combination_only_once}:
    bool. Only has an effect if
    <a href="#randomly_distribute_between_choirs">`randomly_distribute_between_choirs`</a>
...
-   [**`tempo`**]{#tempo}: a number or a sequence of numbers. Specifies
    a tempo or tempi in quarter-note beats-per-minute. If a sequence,
    the length of each tempo segment is set with
    <a href="#tempo_len">`tempo_len`</a>. If a sequence, will be looped
    through if necessary. If an empty sequence, a tempo or tempi will be
    randomly generated.

    *Default*: `120`

-   [**`tempo_len`**]{#tempo_len}: a number or a sequence of numbers.
    Specifies the duration of each tempo in <a href="#tempo">`tempo`</a>
    in quarter-note beats. If a sequence, will be looped through as
    necessary. Has no effect if <a href="#tempo">`tempo`</a> is a single
    number. If 0, then the first tempo in <a href="#tempo">`tempo`</a>
    applies to the whole file.

    *Default*: `0`

-   [**`tempo_bounds`**]{#tempo_bounds}: a tuple of form (number,
    number). If <a href="#tempo">`tempo`</a> is an empty sequence, then
    tempi are randomly generated within the (inclusive) bounds set by
    this tuple.
...
-   [**`transpose`**]{#transpose}: bool. Toggles transposition of
    segments according to the following settings. Note that the limits
    imposed by <a href="#voice_ranges">`voice_ranges`</a> are not
    followed by the transposition settings.

    *Default*: `False`

-   [**`tranpose_type`**]{#tranpose_type}: string. For explanation of
    the possible values, see the discussion of "generic" and "specific"
    transposition above. Possible values:
...
-   [**`transpose_len`**]{#transpose_len}: number, or sequence of
    numbers. The length of the segments to be transposed. If a single
    number, all segments have the same length. If a sequence of numbers,
    will be looped through.

    *Default*: `4`

-   [**`transpose_intervals`**]{#transpose_intervals}: number, or
    sequence of numbers. The interval or intervals by which to transpose
    segments. If a single number, all segments will be transposed by the
    same amount. If a sequence of numbers, will be looped through. The
    transposition is cumulative, so, for example, if
...
-   [**`cumulative_max_transpose_interval`**]{#cumulative_max_transpose_interval}:
    ...
    `cumulative_max_transpose_interval == 6` and
...
-   [**`transpose_before_repeat`**]{#transpose_before_repeat}: boolean.
    If True, then any transpositions are applied to the completed "super
    pattern" *before* repeating it; thus, the super pattern will have
    the same sequence of transpositions on all repetitions. If False,
    then any transpositions are applied *after* repeating the super
    pattern. Thus, unless the sequence of transpositions happen to
