...
def _get_leader_available(er, remaining, leader_rhythm):
    # TODO make this work when the follower rhythm is a different length
    #   (especially longer) than the leader rhythm?
    out = []
    for time in remaining:
        leader_time = time
        while leader_time not in leader_rhythm:
...
def _get_attack_dict_and_durs_to_next_attack(er, voice_i, attack_list):
    ...
    attack_times = {}
    durs = {}
    # time = fractions.Fraction(0, 1) # TODO delete?
...
def _fill_attack_durs(
    ...
    def _fill_attacks_sub(dur_dict, remaining_dict):
        ...
        while target_total_dur > actual_total_dur and available:
            ...
            dur_to_add = min(er.dur_subdivision[voice_i], remaining_dur)
            # TODO What if dur_to_add is less than min_dur?
...
def _add_comma(er, voice_i, attacks):
    ...
    # TODO verify that this is working with complex subdivisions
    rhythm_length = er.rhythm_len[voice_i]
...
def generate_rhythm1(er, voice_i, prev_rhythms=()):
    ...
    if voice_i in er.hocketing_followers:
        ...
        rhythm = er_notes.Rhythm(er, voice_i)
        rhythm.data = attacks
        return rhythm
...
class Grid(er_notes.ContinuousRhythmicObject):
    def __init__(self, er):
        ...
        if len(set(er.num_vars)) > 1:
            raise GridError(
                "Generate grid only works if er.num_vars has a single value."
            )
        self.num_vars = er.num_vars[0]

        if len(set(er.vary_rhythm_consistently)) > 1:
            raise GridError(
                "Generate grid only works if er.vary_rhythm_consistently "
                "has a single value."
            )
        ...
        if len(set(er.cont_var_increment)) > 1:
            raise GridError(
                "Generate grid only works if er.cont_var_increment "
                "has a single value."
            )
        ...
        if len(set(er.min_dur)) > 1:
            print(
                "Notice, more than one value for er.min_dur, using minimum ("
                f"{self.min_dur})"
            )
        ...
        if self.rhythm_len < self.min_dur * self.num_notes:
            ...
            print(
                f"Notice: grid min_dur too long; "
        ...
        # attribute above.
        # TODO revise?
        self.dur_deltas = np.zeros((self.num_vars, self.num_notes))
        for var_i in range(self.num_vars):
            self.dur_deltas[var_i] = (
                self.rel_attacks[var_i]
                - self.rel_attacks[(var_i - 1) % self.num_vars]
            )
        # print(self.dur_deltas)
        # breakpoint()
    ...
    def return_varied_rhythm(self, er, attacks, voice_i):
        ...
        if rhythm_num_notes == 0:
            ...
            return er_notes.ContinuousRhythm(er, voice_i)
        indices = _get_grid_indices()
        var_attacks = np.zeros((self.num_vars, rhythm_num_notes))
        rel_attacks = np.zeros((self.num_vars, rhythm_num_notes))
        # var_attacks[0] = list(attacks.keys())
        var_durs = np.zeros((self.num_vars, rhythm_num_notes))
